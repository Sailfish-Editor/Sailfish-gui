...
const extraSailfishBlocks = `
<block type="argument_reporter_boolean"><field name="VALUE">is compiled?</field></block>
<block type="argument_reporter_boolean"><field name="VALUE">is Sailfish?</field></block>
`;
...
const makeToolboxXML = function (isInitialSetup, isStage = true, targetId, categoriesXML = [],
    ...
    // Always display Sailfish blocks as the first extension, if it exists,
    // and also add an "is compiled?" block to the top.
    let sailfishXML = moveCategory('tw');
    if (sailfishXML && !sailfishXML.includes(extraSailfishBlocks)) {
        sailfishXML = sailfishXML.replace('<block', `${extraSailfishBlocks}<block`);
    }
    ...
    if (sailfishXML) {
        everything.push(gap, sailfishXML);
    }
